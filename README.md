...
20. **自用友好化：一键进入（匿名登录）+ 限流可见**（本轮）：
    ...
    - **多端同步**：新增**邮箱+密码登录**（`POST /auth/v1/token?grant_type=password`）——一个账号在每台设备登一次即可共享同一份数据，不走邮件、不受发信限额；页脚「同步到我的账号」进入（首次进入仍是静默本机身份，零登录界面）。账号只需在 Supabase → Authentication → Users 建一次。
