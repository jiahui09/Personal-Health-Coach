...
```
刊头 → 其一 今日之事 → 其二 下一膳 → 其三 今日之练 → 身体近况（含今日体感） → 近况 → 生活纪事 → 近来手记
```

| 区块 | 功能 |
| --- | --- |
| **刊头与序** | 印章报头与存储标识；「朝安 / 昼安 / 夜安」时段问候 + 干支日期行，一键唤起录事 |
| **其一 今日之事** | 今日待办（增 / 删 / 勾选）、每日随笔（案头小记），行内「添录一事」速记 |
...
| **其二 下一膳** | 基于今日已摄入热量/蛋白质缺口，从餐食模板库中排序推荐，给出**热量区间与蛋白质区间**（而非假装精确的单一整数），「照准」一键记账 /「缘由」查看决策稽核 |
...
| **生活纪事** | 本周时间投入（写码 / 研学 / 习练 / 披阅）纯事实 + 近期手记，可按分类筛选 |
| **录事弹层** | 底部弹层，四签：进食 / 习练 / 体征 / 手记 |
| **决策稽核弹窗** | 「缘由」——全部内容来自引擎真实 props：君之实录、所中之规、决策之迹（输入快照 / 推导值 / 前提 / 局限）、证据所出与工程按语 |
...
| **时间窗** | `getMeals/getWorkouts/getLifeLogs/getWeightHistory/getTodos/getNotes(since?)` | 不再全量拉历史；Supabase 上等价于 `where date >= since` |
...
- **原始记录**：`WeightRecord`（带 `source`/`time`）、`MealRecord`（带 `source`/`confirmed`）、`WorkoutRecord`（带 `category`/`durationSource`）、`DailyState`（`sleep` 为判别联合：`interval` 或 `duration`，字段可缺席）、`TodoItem`（`status`）、`LifeLog`（`content` 可空）、`DailyNote`。
...
- **窗口不混用**：今日任务/饮食/训练/体重/体感用 Today；「本周之功」与抗阻进度用 **ThisWeek（周一起）**；七日均重、睡均、近来手记用 **Last7Days（滚动）**；端点变化、斜率、记录条数用 **Last30Days**。
...
- **标题全中文、古记事体**：章节头统一由 `src/components/SectionHead.tsx` 输出：**眉行（章序「其一 / 其二 / 其三」＋右注，固定 18px）→ 题行 26px 衬线 → 2px 墨线压底**，右栏附目与通栏同规格；眉行右端之外，章节右侧另有 36px **旁批槽**，朱色竖排小字写裁决（如「已成其一」「照减脂之期」「抗阻合议」），窄屏降级为题下横排朱批，日期行是干支年 + 中文月日 + 星期（如 `丙午年 · 九月廿五 · 星期五`，由时钟推导，1984 年为甲子），刊头是朱红「记」印章 + 「个人健康手记」报头，刊头与页脚各压一条 2px 墨线。
...
- **动作三级体系，朱只表裁决与状态**：`.btn-primary`（墨实心：录一笔 / 照准 / 毕此一练 / 览毕）→ `.btn-ghost`（描边：缘由）→ `.btn-link`（文字链：别录一品 / 另择动作 / 添录一事 / 记此一刻 / 复其初等），类定义在 `src/index.css @layer components`。**朱只用于「已成 / 达标 / 御批 / 图表」**：勾选框、完成徽标、Toast 勾、达标句（合议 · 每周两日抗阻 / 合议 · 七时之基）、状态词（健 / 微酸）、旁批、计量条达标填充与体重折线；章序与一切动作型文字/图标改为墨色。
...
- **全页只留一张图表，其余数字入句**：唯一图表是 30 日体重折线（`TodayData.weightSeries`，手画 SVG，来自真实记录、不造序列）；摄入、趋势、预测区间都写成手记行文，精力与酸痛用两行对齐的 1–5 圆点点按——**没有 KPI 卡、没有瓦片墙、没有环形进度**；比例信息只用**墨线计量条**（`RuleMeter`：3px 直角细线，底轨 `--color-line`，填充墨＝进行中 / 朱＝达标 / 绛＝超录，无圆角阴影渐变动画），百分比与分子分母一律以真实文本入句（蛋白质 63/110 g · 57%、抗阻 2/2 · 100%、睡均 7.3/7 h · 104%、生活纪事占本周）。
- **版式**：容器 1100px；桌面双栏**按行配对**（`lg:col-start`/`lg:row-start` 显式落位，三对章节横线跨栏同 y，中缝一条 1px 折缝线），左＝章目其一/其二/其三，右＝近况 / 生活纪事 / 身体近况＋体感（右栏次序按等高配对重排，故与移动端不同），近来手记与页脚通栏；**移动端单栏仍按原顺序**（其一 → 其二 → 其三 → 身体近况 → 近况 → 生活纪事），章节级动作统一收在章末「动作脚注行」（`.section-actions`），窄屏整行另起、不与正文并排。
- **文案全取古风行文，不用现代白话**：时段问候作「朝安 / 昼安 / 夜安。」，主句作「凡四事，已成其一。」；章节作「下一膳 / 今日之练」，动作作「录之 / 罢 / 照准 / 毕此一练 / 览毕」，危险动作作「掷还」，报错作「膳食之录未成（code）」，反馈作「知道了 · 一事已列入今日之册」（成功回执一律冠「知道了 ·」，失败不冠），达标作「合议 / 未合议 · 尚差二日」，页脚作「存于本机 · 不假模型 · 规则可稽」；记录表单、稽核弹窗结构标签（君之实录 / 所中之规 / 决策之迹 / 证据所出 / 工程按语）同此体例。**保留不改的只有三类**：文献引文与 `claim`/`limitations`（`scientificEvidence.ts`，学术原貌即证据本身）、规则 ID 与 ASCII 单位符号（kcal / kg / AASM / RIR）、食物与练习的中文专名。
...
- **体验完整**：单页流、Toast 反馈、底部记录弹层、证据弹窗、演示数据重置，中文文案统一克制。

### 本轮修复清单（去 AI 味 + 原型 → 工程化）

1. **证据弹窗改为真实渲染**：`EvidenceModal` 不再展示写死的 `Weight: 68.4 kg` / `PROTEIN_TARGET_01`，而是把 `trace`、`evidenceTraces`、`ruleId/ruleStatus`、`equipmentNote/translationNote` 等 props 完整呈现（你的记录 / 命中规则 / 决策轨迹 / 证据来源 / 工程说明五段）。
...
11. **文案整体改为古风行文**（本轮）：覆盖全部组件标签与按钮、Toast 与报错、记录表单与占位、稽核弹窗结构标签、引擎推荐语与恢复指引、演示数据（待办/随笔/手记条目）；文献引文与规则 ID 按上述三例外保留，`tsc`、9/9 测试、`kill-ai-slop` 0 hits 均复验通过。
