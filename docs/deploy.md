...
一个账号、每台设备登一次即可（**不发邮件、不受发信限额**）：

1. **建账号（只做一次）**：Supabase → Authentication → **Users** → **Add user**
   填你的邮箱与密码，勾选 **Auto Confirm User**（或建好后在用户菜单里点 Confirm）。
2. **每台设备各登一次**：打开站点 → 直接进入应用（本机身份）→ 页脚点 **「同步到我的账号」**
   → 填邮箱密码 → 登录。之后这台设备就用账号身份读写数据。
3. 第二台设备重复第 2 步 → 两台设备看到的是**同一份记录**。
4. 登录状态存本机并自动续期，日常不需要反复登；想退出就清除该站点的浏览器数据。

注意：
- **先登录再录数据**。本机身份名下已录的内容不会自动并入账号（是两套 `user_id`）；
  若已经录了，先在原设备点「同步到我的账号」——那份数据的归属仍是旧身份，需要手工搬（目前未提供自动迁移）。
- 账号密码存在 Supabase 的 Auth 里，应用不会保存明文密码，只保存会话令牌。
- 想再加一层保险（忘记密码时用）：把 Email provider 也开着，用「改用邮箱登录」收链接重置。
